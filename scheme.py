...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        if isinstance(operator, MacroProcedure):
            expr = complete_apply(scheme_apply(operator, rest, env))
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('lengths of arguments and formals do not match')
        else:
            while formals != nil:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        f, v = bindings.first.first, bindings.first.second.first
        formals = Pair(f, formals)
        vals = Pair(scheme_eval(v, env), vals)
